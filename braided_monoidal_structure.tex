...
In the following we will construct a braided monoidal structure obtained from an op-fibration over $\Br$.
To do so, we will need to choose paths of configurations in $\Br$ on various occasions. We will refrain from specifying
...
$\pi \colon \Cotimes \to \Br$ such that $\pi$ is an op-fibration and the fibers 
...
always refers to an active morphism in $\Br$ from now on for all $n \in \N$.

\begin{defin}
    An $\Br$-operad is a category $\Cotimes$ with a functor
    \[
        \Cotimes \to \Br
    \]
    satisfying
...
So for the next part, let $\pi \colon \Cotimes \to \Br$ be a $\Br$-operad and we denote
...
For a $2$-morphism $\eta$, i.e. a path between two configurations $c_1, c_2 \in \E_2(n)$ in $\Br$,
...
\begin{align*}
    ...
    \end{cases}
\end{align*}
We consider these morphisms in $\Br$ with the following collection of configurations:
\begin{align*}
...
For the associator we consider the following diagram in $\Br$:
    \[
    \begin{tikzcd}
...
$\eta \colon \langle 0 \rangle \to \langle 1 \rangle$ in $\Br$ 
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \]
    \end{enumerate}
    Each edge in the hexagon diagrams refers to a commuting diagram in $\Br$ of the form
    ...
    another while filling the tetrahedron in $\Br$ means having a homotopy between the different paths. 
    ...
    Analoguously for the hexagon identity $(2)$, we consider the following composition of paths in $\Br$:
    ...
    \begin{center}
        ...
        \end{tikzpicture}
    \end{center}
    This concludes the proof that $\Br$ induces a braided monoidal structure on $\C$.

...
In the following we will construct a braided monoidal structure obtained from an op-fibration over $\Ex$.
...
choose a path so we then see why certain compositions of paths are homotopy equivalent. 
Also, we do not care much about the size of the
...
We will now follow the same approach as in the symmetic monoidal case in section 
\ref{sec:operads}, i.e. we will define a category $\Cotimes$ and a functor 
$\pi \colon \Cotimes \to \Ex$ such that $\pi$ is an op-fibration and the fibers 
...
is induced by $\otimes \circ \beta'$. 
They are inverse to one another since lifts with chosen source are unique.
...
\begin{proof}
    ...
    So, we can arbitrarly translate and resize configurations with middle points on the horizontal axis,
    ...
    another while filling the tetrahedron in $\Ex$ means having a homotopy between the different paths. 
    ...
    Let us understand where the diagram or more precisely the configurations come from.
    ...
    Similar arguments for all the corresponding diagrams induced by the hexagon identity $(1)$
    give us paths that are already defined by the associativity path and the half twist up to homotopy.
        \begin{center}
        \begin{tikzpicture}
            %arrows

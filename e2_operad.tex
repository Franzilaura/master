...
In the following we will construct a braided monoidal structure obtained from a cofibration over $\Ex$.
...
So, for this section we assume a basic topological understanding which for instance can be found in \todo{topo reference?}
...
We will follow now the same approach as in the symmetic monoidal case in section \ref{sec:operads}, i.e. we will define a category $\Cotimes$ and a functor $\pi \colon \Cotimes \to \Ex$ such that $\pi$ is a cofibration and the fibers of $\pi$ are given by the categories $\C^n \coloneqq \Cotimes_{\langle n \rangle}$ for $n \geq 1$.
...
\begin{defin}
    ...
    satisfying
        \begin{enumerate}[(M1)]
            \item The functor $\pi$ is a cofibration.
            \item For $\C \coloneqq \Cotimes_{\langle 1 \rangle}$, we have $\Cotimes_{\langle n \rangle} \cong \C^n$ for all $n \geq 1$. 
        \end{enumerate}
...
Since $\pi$ satisfies (M1) and (M2), we have that for any $(A,B) \in \C^2 \cong \Cotimes_{\langle 2 \rangle}$ 
there is a cocartesian lift $(A,B) \to A \otimes B$. This choice is unique up to isomorphism. 
...
\todo{define at least half twist in more detail? and then skip details for the rest of the paths?}
\textcolor{blue}{How much detail is necessaary? giving every path seems tedious and not beneficial to the reader..}
\todo{fix half twist: wrong direction}
\begin{center}
    \begin{tikzpicture}
                        % basepoint
                        \node at (1, 0) {\small $2$};
...
\begin{proof}
    ...
    Hence, we focus on the hexagon identities
    \begin{enumerate}
        \item 
        \[
            \begin{tikzcd}
...
\[
    \E_2 \to \E_2 \slash S
\]
We identify configurations up to relabeling. 
...
Observe that that for every $\gamma \in \widetilde{\E_2(n)} \slash S_n$ there is a commuting diagram in $\Ex$ for some $\sigma \in S_n$:
\begin{center}
    \begin{tikzcd}
...
\begin{proof}
    ...
    on configuration in the $l$-th disk of $b_k$ for all $1 \leq l \leq k$
    and then apply 
    ...
    \todo{drawing of what is happening here.}
    Further, let $\tau \in \E_1(j) \subset \E_2(j)$ be a path from $b_j$ to $\gamma((b_k, b_{j_1}, \dots, b_{j_k}))$ 
    that resizes and translates the disks along the horizontral axis. This path $\tau$ is unique up to homotopy.
    ...
    Furthermore, we define a operadic unitality 
    \[
...
\end{proof}
    ...
    This construction of an operadic structure on the universal covering of a topological operad can me given in a more general setting.
    Which we did not require at this point. ... \todo{reference to the general construction or Adrians lemma}.

    \todo{braided action on the universal covering of $\E_2$}
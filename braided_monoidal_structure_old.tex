...
To do so, we will need to choose 2-morphisms in $\Brx$ on various occasions.
...
\begin{defin}
    An $\Brx$-monoidal category \todo{change naming} is a category $\Cotimes$ with a functor
    \[
        \Cotimes \to \Brx
    \]
    satisfying
        \begin{enumerate}[(M1)]
            ...
            we have $\Cotimes_{\langle n \rangle} \cong \C^n$ for all $n \geq 1$. 
        \end{enumerate}
...
We will write the following diagram instead:
...
For the associator we consider the following diagram in $\Brx$:
    \[
    \begin{tikzcd}
        \langle 3 \rangle \arrow[r, "\tau_2^3"] \arrow[d, "\tau^3_1"] & \langle 2 \rangle \arrow[d, "\mu"] \\
        \langle 2 \rangle \arrow[r, "\mu"]                                   & \langle 1 \rangle          
    \end{tikzcd}
    \]
    ...
    \todo{$\gamma$ lies in a sense in $\E_1$ and we do not care too much about $\E_1$...}
...
\begin{proof}
    ...
    \todo{rewrite associativity, refer to $\E_1$ translation in 3.2}
    ...
    another while filling the tetrahedron in $\Brx$ means having a homotopy between the different paths. 
    ...
    Analoguously for the hexagon identity $(2)$, we consider the following composition of paths in $\Brx$:
...
\todo{if time: Define colored braided operad from a braided mon cat.}
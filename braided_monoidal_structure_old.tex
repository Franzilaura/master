...
braided monoidal structure on a category $\C$ that is induced by an op-fibration
...
the paths enough to understand from which homotopy class we choose a representative path so we
...
For the next part, let $\pi \colon \Cotimes \to \Brx$ be a $\Brx$-monoidal
category and we denote $\C \coloneqq \Cotimes_{\langle 1 \rangle}$. In this
construction we aim to define a braided monoidal structure on $\C$. First,
...
Instead, we write dashed arrows for the $2$-morphism as in the
diagram:
...
\begin{proof}
    ...
    First, the pentagon identity holds as compositions of $\E_1$-translations are
    ...
    Analogously for the hexagon identity $(2)$, we consider the

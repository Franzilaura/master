...
satisfies the Segal condition. In this section, we will define a braided
monoidal structure on a category $\C$ that is induced by an op-fibration over
the category $\Brx$ also satisfying the Segal condition. To do so, we will need
to choose 2-morphisms in $\Brx$ on various occasions. We will refrain from
...
\E_2(n)$ in $\Brx$, we would write the following diagram:
\begin{center}
    \begin{tikzpicture}
        % Define node positions

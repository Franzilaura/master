...
by translation and resizing which are pairwise disjoint. We use topological
terms when talking about $\E_2$ what we really mean is applying the term
level-wise. We will see that the universal coverings of $\E_2(n)$ for all $n \in \N$
can be equipped with an operadic structure as well as a braid action. Hence, we
...
differ in their labels are identical. It will be useful to identify configurations that only differ in their labels.
...
The following lemma will be essential to establish a braid group action on 
...
\begin{lem}\label{lem:fundamentalgroup} The following statements hold for every
    ...
    \begin{enumerate}
        ...
        \item The fundamental group of $\E_2(n) \slash S_n$ is the braid
        group $B_n$.
    \end{enumerate}
\end{lem}
...
In the next step, we will first define a universal covering of $\E_2$ and $\E_2 \slash S$ for each level $n$.
Then, we will see that these spaces are homeomorphic on each level. This allows us to transfer the braid action on
...
\begin{rem}
    ...
    \dots, j_p^{(k)}\right\}$. So, with the composition map $\gamma$ we obtain a configuration 
    \[
...
So, \todo{do I need this comment?}
in $\Ex$ we identify $f, g \colon \langle n \rangle \to \langle m \rangle$
...
accurate, but our notation will be more convenient in this part.
\begin{center}
    \begin{tikzpicture}
...
\begin{proof}
    ...
    horizontal line. Hence, it no braid crossing. Further, we know that composition of paths
    ...
    $\bar{\alpha} \colon b_k \to c$. Clearly, the unitality condition is also
    met by definition:
    \begin{center}
        \begin{tikzcd}
...
\begin{rem}
    ...
    topological operad as long as there is a suboperad of $\E_1$ into the
    topological operad.
\end{rem}
...
S(n)$, we obtain now the full $n$-th braid group $B_n$. Since, there is a
...
basepoint configuration to another configuration once and we do not touch the

...
levelwise universal coverings of $\E_2$. We recall the operad $\E_2$ from
...
by translation and resizing and which are pairwise disjoint. We use
topological terms when talking about $\E_2$ what we really mean is applying the
term level-wise. The universal covering of $\E_2$ will be another example of a
braided operad because of the following fact:
...
\begin{lem}\label{lem:fundamentalgroup} The following statements hold for every
    ...
    \begin{enumerate}
        ...
        \item The fundamental group of $\E_2(n)$ without labels is the braid
        group $B_n$.
    \end{enumerate}
\end{lem}

As the fundamental group acts on the universal covering of a space we then
obtain a braid action on the universal covering of $\E_2$. We will not prove the
lemma here. A proof by Fox and Neuwirth can be found in \cite{fox}. However, we
will give a sketch of why this is the case at the end of the section.

Before we get started, we need to define what it means to be a universal
covering of a topological operad.

\begin{defin}
    Let $\P$  be a topological operad. A covering operad $\widetilde{\P}$ is a
...
\begin{defin}
    Let $\P$ be a topological operad. A universal covering operad
    $\widetilde{\P}$ is a covering operad that is universal in every degree $n
    \geq 1$.
\end{defin}

The operadic structure requires labels as they are necessary to define the
composition maps in $\E_2$. However, topologically configurations, or more
precisely the images of the disk embeddings of configurations, are identical if
the configurations only differ in their labels.
\todo{clear what is meant by that?}
Nevertheless, when we want to talk about universal covers and fundamental groups
we need to choose a basepoint. So formally, for any choice of a basepoint on
each level $n$, permuting its labels will give us a different point in
$\E_2(n)$. Hence, if we consider the fundamental group level-wise over each
basepoint we only obtain the pure braid group on $n$ strands by Lemma
\ref{lem:fundamentalgroup}. In this section we want to establish a universal
covering of $\E_2$ without labels which has the full braid action. We will see
in the next step that this universal covering is level-wise homeomorphic to the
universal covering of $\E_2$ which will allow us to transfer the braid action on
the universal covering of $\E_2$ without labels to the universal covering of
$\E_2$ with labels.
...
\[
    \E_2 \to \E_2 \slash S,
\]
i.e. we identify configurations that only differ in their labels. In this part,
we will work a lot with $\Ex$ which is Definition \ref{def:Ox} applied to
$\E_2$. This gives us a nice way to talk about configurations and paths between
...
\begin{rem}
    ...
    For $\sigma \in S_n$ we refer to the corresponding automorphism of $\langle n \rangle$
    with configurations $(\id^1, \dots, \id^n)$ also by $\sigma$.
    ...
    \dots, j_p^{(k)}\right\}$. So, we obtain a configuration 
    \[
...
\begin{eg}
    ...
    \end{center}
\end{eg}
The category $\Ex$ will be an essential ingredient to establish colored braided
operads.
...
\begin{center}
    ...
    \end{tikzpicture}
\end{center}
ahh you mean "since E2(n) is connected and simply connected, the quotient is
connected!"
...
$\widetilde{\E_2(n) \slash S_n}$ which we define:
\[
...
of $\E_2(n) \slash S_n$ is shown in Section 1.3 in \cite{Hatcher}. Analoguously,
there is a universal covering of $\E_2$ with labels, when we choose the
...
\begin{align*}
    ...
    \gamma &\mapsto \gamma(1)
\end{align*}
The labels have no purpose from a topological perspective, so it should not come
as a surprise that there is a homeomorphism for each $n \in \N$
\begin{align*}
...
induced by the path $\gamma$. Clearly this is continuous in both directions and
inverse to one another. This shows that topologically the path $\gamma$ does not
need the labels which is summarized in the following notation.
For a path $\gamma \colon \bar{b_n} \rightsquigarrow c$ between configurations $\bar{b_n}, c \in \widetilde{\E_2(n)} \slash
S_n$ we write the following diagram for some $\sigma \in S_n$:
\[    
    \begin{tikzcd}
...
configurations in $\E_2(n)$. Hence, the following diagram would be more accurate,
but our notation will be more convenient in this part.
\begin{center}
    \begin{tikzpicture}
...
then also use it to define a braid action on the universal covering.

First, we notice a specific translation between specific configurations that are
unique up to homotopy equivalence.

\begin{rem}[$\E_1$ translation]\label{rem:e1-translation} First, we observe that
    ...
    unique up to homotopy equivalence. This follows from the fact that the ordered
    configuration space of the real line
    \[
    ...
    \]
    Since the open simplex $\inter(\Delta^{n-2})$ is contractible then so
    is $\Conf_n(\R)$.
    \todo{source + continue}
...
\begin{proof}
    ...
    horizontal line. Hence, it has no proper effect from a homotopy perspective
    as it creates no braid crossing. Further, we know that composition of paths
    is associative up to homotopy. So, for 
    ...
    \]
    to be the trivial path of the trivial basepoint $b_1 \in \E_2(1)$. Clearly,
    ...
    $\bar{\alpha} \colon b_k \to c$. Clearly, the unitality condition is also met
    by definition:
    \begin{center}
        \begin{tikzcd}
    ...
    structure and the covering map is a morphism of operads.
\end{proof}

\begin{rem}
    This construction of an operadic structure on the universal covering of a
    topological operad can be given in a more general setting, which we did not
    require at this point. But in \cite[Lemma 4.1.12]{adrian} you can find a
    more general statement on how to construct an operadic structure on the collection
    of level-wise universal coverings of a sufficiently connected topological operad as long as there is a suboperad
    of $\E_1$ into the topological operad.
\end{rem}
...
level-wise homeomorphic to the universal covering of $\E_2$. As usual, there is a group
action of the fundamental group of $\E_2(n) \slash S(n)$ on its universal
covering. The fundamental group of $\E_2(n)$ is the $n$-th pure braid group
$PB_n$. But since we forgot the labels when moving to $\E_2(n) \slash S(n)$, we
obtain now the full $n$-th braid group $B_n$. Since, there is a homeomorphism
$\widetilde{\E_2 \slash S} \cong \widetilde{\E_2}$, we can lift the braid group
action to $\widetilde{\E_2}$. For a detailed proof, we refer to the paper of
Fox and Neuwirth \cite{fox}.
However, we give a short overview of the construction here.
...
$i$-th and $(i+1)$-th disk in $\widetilde{\E_2 \slash S}$ which we will also
...
What is left to show is that this braid action is equivariant with respect to
the operadic structure on $\widetilde{\E_2}$. \textcolor{blue}{Note that by definition of
$\widetilde{\gamma}$ we move disks in a large disk $D$ from the basepoint
configuration to another configuration once and we do not touch the
configuration of $D$ again afterwards but only move $D$ as a part of another
configuration.} Hence, in that sense the order in which we apply paths to a
configuration does not matter from a homotopic perspective. This fact is

...
\section{Operads}\label{sec:operads}
    ...
    certain number of inputs, and a way to compose these operations. 
    ...
    So, for an ordered family of loops $\{\alpha_i \colon I_i \to X\}_{1 \leq i \leq n}$ there is 
    a unique map $f \in \Emb(I_1, \dots, I_n, I)$ for each order preserving concatenation $\beta$ 
    such that $\beta \circ f (I_i) = \alpha_i$ for all $1 \leq i \leq n$. We denote the set of 
    embeddings by $\E(n) = \Emb(I_1, \dots, I_n, I)$ for each $n \in \N$.
    For every $(f_k, (f_{j_i})_{1 \leq i \leq k}) \in  \E(k) \times \E(j_1) \times \dots \times \E(j_k)$ 
    we obtain an embedding $f \in \E(j_1 + \dots + j_k)$ by defining 
    $f = f_k \circ (f_{j_1} \sqcup \dots \sqcup f_{j_k})$. This defines a composition map 
    \[
    ...
    \begin{center}
        \begin{tikzcd}
            \E(k) \times \left(\prod_{s=1}^k \E(j_s)\right) \times \left(\prod_{r=1}^j \E(i_r)\right) \arrow[rr, "\gamma \times \id"] \arrow[dd, "s"']               &  & \E(j) \times \left(\prod_{r=1}^j \E(i_r)\right) \arrow[d, "\gamma"]   \\                                                                           &  & \E(i)   \\
            \E(k) \times \left(\prod_{s=1}^k \left(\E(j_s) \times \left(\prod_{q=1}^{j_s} \E(i_{g_{s-1}+q})\right)\right)\right) \arrow[rr, "\id \times \gamma^s"'] &  & \E(k) \times \left(\prod_{s=1}^k \E(h_s)\right) \arrow[u, "\gamma"']
        \end{tikzcd}
    \end{center}
    ...
    \begin{defin}\label{planaroperad}
        ...
        \begin{itemize}
            ...
            the following diagram commutes:
                \begin{center}
                \begin{tikzcd}
                    \P(k) \otimes \left(\bigotimes_{s=1}^k \P(j_s)\right) \otimes \left(\bigotimes_{r=1}^j \P(i_r)\right) \arrow[rr, "\gamma \otimes \id"] \arrow[dd, "s"']               &  & \P(j) \otimes \left(\bigotimes_{r=1}^j \P(i_r)\right) \arrow[d, "\gamma"]   \\                                                                           &  & \P(i)   \\
                    \P(k) \otimes \left(\bigotimes_{s=1}^k \left(\P(j_s) \otimes \bigotimes_{q=1}^{j_s} \P (i_{g_{s-1}+q})\right)\right) \arrow[rr, "\id \otimes (\otimes_s \gamma)"'] &  & \P(k) \otimes \left((\bigotimes_{s=1}^k \P(h_s)\right) \arrow[u, "\gamma"']
                \end{tikzcd}
                \end{center}
    ...
    \begin{egs}\label{egs_planar_operad}
        ...
        \begin{enumerate} 
            \item The \textit{associative operad} $\Ass$ is the operad whose algebras 
            are associative monoids. We will define what it means to be an algebra over an operad 
            later. However, the associative planar operad in $\C$ is 
            ...
            Let $\C = \Top$. Then we can define the \textit{operad of little intervall} $\E$ as
            \[
            ...
            \begin{center}
                \begin{tikzcd}
                    {\HHom\left(X^{\otimes k}, X\right) \otimes \bigotimes_s \HHom\left(X^{\otimes j_s}, X\right)} \arrow[d] \\ {\HHom\left(X^{\otimes k}, X\right) \otimes \HHom\left(X^{\otimes j}, X^{\otimes k}\right)} \arrow[d] \\
    ...
    For another example of an operad, which we will encounter a lot throughout this thesis, we need to recall the definition of the braid group first.
    ...
    \begin{eg}\label{braidop}
        ...
        Let us familirize ourselves with the composition by looking at an example.
        For the compositon map 
        \[
    ...
    \begin{defin}\label{operad}
        A {\em (symmetric) operad} $\O$ in $\C$ is a planar operad as defined in definition \ref{planaroperad} with an action of $S_n$ on $\O(n)$ together with the following equivariance condition: 
           Let $\sigma(j_1, \cdots, j_k) \in S_j$ denote the element that permutes $k$ blocks of length $j_l$ for $1 \leq l \leq k$. Then, the following diagram commutes:
            ...
            Furthermore, let $\tau_l \in S_{j_l}$ and $\tau_1 \oplus \cdots \oplus \tau_k$ denote the sum of permutations on $k$ blocks of length $j_l$ for $1 \leq l \leq k$. Then the following diagram commutes:
            ...
            \item Associativity:
                \begin{center}
                    \begin{tikzcd}
                        ...
                        &  & A  \\
                        \O(k) \otimes \left(\bigotimes \left(\O(j_l) \otimes A^{j_l}\right)\right) \arrow[rr, "\id \otimes \theta^k"']               &  & \O(k) \otimes A^k \arrow[u, "\theta"']
                    \end{tikzcd}
                \end{center}
            \item Unitality:
                \begin{center}
    ...
    \begin{defin}\label{module}
        ...
        \begin{itemize}
            \item Associativity: For all $\sum j_l = j$ in $\N$ the following diagrams commute:
                \begin{center}
                \begin{tikzcd}
            ...
            \item Equivariance: For all $j \in \N$ and $\sigma \in S_{j-1} \subset S_j$, the following diagram commutes:
                \begin{center}
                \begin{tikzcd}
    ...
    \begin{rem}
        We can of course also define algebras over planar operads by dropping the equivariance condition as there is no symmetric group action. So, algebras over the operad $\Ass$ which we defined in examples \ref{egs_planar_operad} are indeed monoids since the corresponding squares commute due to canonical isomorphisms:
            \begin{center}
                \begin{tikzcd}
                    \Ass(2) \otimes \Ass(2) \otimes \Ass(1) \otimes A^3 \arrow[rr, "\cong"] \arrow[dd, "\cong"'] &  & \Ass(3) \otimes A^3 \arrow[d, "\theta"]  \\
                    &  & A  \\
                    \Ass(2) \otimes \left(\left(\Ass(2) \otimes A^2\right) \otimes \left(\Ass(1) \otimes A \right)\right) \arrow[rr, "\id \otimes \theta^2"']               &  & \Ass(2) \otimes A^2 \arrow[u, "\theta"']
                \end{tikzcd}
            \end{center}  
            ...
            \begin{center}
                \begin{tikzcd}
                    ...
                    &  & A  \\
                    \Ass(2) \otimes \left(\left(\Ass(1) \otimes A \right) \otimes \left(\Ass(2) \otimes A^2\right)\right) \arrow[rr, "\id \otimes \theta^2"']               &  & \Ass(2) \otimes A^2 \arrow[u, "\theta"']
                \end{tikzcd}
            \end{center}  
    \end{rem}
    ...
    \begin{egs}
        \begin{enumerate} \label{eg:operads}
            ...
            \]
            is the closed unit $n$-disk. A \emph{little $n$-disk} is defined as the embedding $f \colon \dd^n \to \dd^n$ of the following form:
            \[
            ...
            \begin{itemize}
                ...
                \item After placing all configurations inside the $k$ disks, the boundaries of the $k$ disks are erased, yielding a new configuration in $\dd_n(n_1 + \dots + n_k)$.
            \end{itemize}
            ...
            \end{align*}
            The little $n$-disk operad $\dd_n$ is a model of the $\E_n$ operad and specifically $\E_2$
            will be of great importance to us throughout the entire thesis. There are other models for the $\E_n$ operads.
            In fact there are recognition principles for $\E_n$ operads, which will be the motivation
            for section \ref{sec:braidedoperads}. However, we will not discuss these other models here.
            So, when we talk about $\E_n$ we are referring to the little $n$-disk operad $\dd_n$.
        \end{enumerate}
    \end{egs}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    \begin{defin}
        ...
        \begin{center}
            \begin{tikzcd}
            z \arrow[r, "g"] \arrow[d, "\exists! \omega'"', dotted] & y & {} \arrow[r, "P", shift right=9] & {} & P(z) \arrow[r, "P(g)"] \arrow[d, "\omega"'] & P(y) \\
            x \arrow[ru, "f"']                              &   &                                  &    & P(x) \arrow[ru, "P(f)"']          &     
            \end{tikzcd}
        \end{center}
    ...
    \]
    where $\Hom_{\C}(y,z) \times_{\Hom_{\D}(P(y), P(z))} \Hom_{\D}(P(x), P(z))$ is the pullback of applying 
    the functor $P$ to $\Hom_{\C}(y,z)$ and precomposing with $P(f) \colon P(y) \to P(x)$.
    So, for the bijection we send a pair $(g, \omega)$ to the unique morphism $\omega'$ and vice versa we send 
    a morphism $h \colon x \to z$ to the pair $(h \circ f, P(h))$.

    \begin{defin}
        Let $P \colon \C \to \D$ be a functor. $P$ is called a {\em Grothendieck fibration} if for every object 
        $y$ in $\C$ and morphisms $f' \colon x' \to P(y)$ there exists a cartesian morphism $f \colon x \to y$ 
        such that $f' = P(f)$.
        We call $f$ a cartesian lift of $f'$ and $f'$ a cleavage.
        Dually we define the notion of a {\em Grothendieck op-fibration}, i.e. $P$ is a Grothendieck 
        op-fibration if for every object $x$ in $\C$ and morphism $f' \colon P(x) \to y'$ there exists a 
        cocartesian morphism $f \colon x \to y$ such that $P(f)=f'$.
    \end{defin}

    If not stated otherwise, when we talk about fibrations and op-fibrations in this thesis, we will 
    always mean Grothendieck fibrations and Grothendieck 
    op-fibrations, respectively. Also note, that in \cite{kerodon}, these termes are refered to as 
    cartesian and cocartesian fibration. 
    However, we chose to use the terminology used in \cite{ha}.
    
    In the next part we will construct a functor having certain properties from a symmetric monoidal 
    category. We will then see that the functor encodes all the necessary data of the symmetric
    monoidal category.
    ...
    \begin{defin}\label{Cxdef}
        ...
        \begin{itemize}
            ...
            \item A morphism $f \colon [C_1, \dots, C_n] \to [C_1', \dots, C_m']$ in $\Cotimes$ consists of a subset $S_f \subset \{1, \dots, n\}$, a map of finite sets $\alpha_f \colon S_f \to \{1, \dots, m\}$ and a family of morphisms
    ...
    \end{defin}
      ...
      which sends a sequence $[C_1, \dots, C_n]$ to $\langle n \rangle$ and 
      a morphism $f \colon [C_1, \dots, C_n] \to [C_1', \dots, C_m']$ to 
    ...
    \begin{rem}
          Let $\C_0$ denote the trivial category of one object and one morphism. We 
          claim that the forgetful functor $P \colon \C_0^{\otimes} \to \Fin$ is an 
          isomorphism of categories. This holds, since for all morphisms in $\C_0^{\otimes}$ 
          the family of morphisms $f_j$ in $\C_0$ is trivial, in other words there is nothing to forget. Hence, $\Fin$ is obtained as the simplest example of the construction of a $\Cotimes$ category.
    ...
    \begin{prop}\label{m1m2}
        Let $P \colon \Cotimes \to \Fin$ be the forgetful functor as describes above. Then, the following conditions hold:
            \begin{enumerate}[(M1)]
                \item \textbf{Op-fibration condition:} $P$  is a Grothendieck op-fibration. 
                \item \textbf{Segal condition:} We have that $\C \cong \Cotimes_{\langle 1 \rangle}$ and more precisely we obtain that $\Cotimes_{\langle n \rangle} \cong \C^n$ 
                where $\C^n$ denotes the $n$-fold product of  copies of $\C$. 
            \end{enumerate}
    \end{prop}
    ...
    \begin{proof}
        \begin{enumerate}[(M1)]
            ...
            First, we will show that for any morphism $\langle n \rangle \to \langle m \rangle$ the op-fibration condition (M1) 
            ...
            an object in $\Cotimes_{\langle n \rangle}$. By the op-fibration condition (M1), we can find a cocartesian morphism 
...
its forgetful functor $P \colon \Cotimes \to \Fin$ satisfiesthe above conditions. In other words, for 
any category $\D$ together with a functor $P \colon \D \to \Fin$ satisfying the op-fibration condition (M1) and the Segal condition (M2), we can construct
...
The right candidate for $\C$ is clearly $\D_{\langle 1 \rangle}$.
 ...
 \begin{prop}\label{SymMonBijection}
    The construction of $\pi \colon \Cotimes \to \Fin$ from a symmetric monoidal category $\C$ given above is a bijection:
    \[
 ...
 \begin{proof}
    Let $\D$ be a category and $P \colon \D \to \Fin$ a functor satisfying the op-fibration condition (M1) and the Segal condition (M2). We will equip 
...
\begin{rem}
    ...
    \]
    This functor is an op-fibration. The proof in Proposition 
    \ref{m1m2} applies. Note that for the proof the fact 
    that morphisms in $\deltast$ preserve the order is essential, otherwise we could not define
    \[
             \omega_j \colon \bigotimes_{\alpha_{\omega}(k)=j} C_k' \cong \bigotimes_{\alpha_{\omega}(k)=j} \bigotimes_{\alpha_f(i)=k} C_i \cong \bigotimes_{\alpha_g(k)=j} C_k \xrightarrow{g_j} C_j''
            \]
    in a monoidal category $\C$.
    Further, the Segal condition (M2) holds as it is a result of the 
    op-fibration condition (M1) and the fact that $\rho^i$ is 
    a morphism in $\deltast$.
    It then also follows that this similar construction is again a bijection
    \[
    \{\text{monoidal categories}\} \cong \{\text{op-fibration } \D \to \deltast \text{ satisfying (M2)}\}.
    \]
    The proof for this is analoguous to Proposition \ref{SymMonBijection} as 
    all morphisms in $\Fin$ that induce a 
...
that, similarly to describing 
a symmetric monoidal structure via a functor satisfying the op-fibration 
condition (M1) and Segal condition (M2), there is also a functor satisfying 
...
\begin{defin}\label{coloroperad}
    ...
    \begin{enumerate}[(i)]
        ...
        \begin{center}
            \begin{tikzcd}
                {\Mul_{\O}(X_I, Y)} \arrow[r, "\cong"] \arrow[d, "\id"] & {\Mul_{\O}(X_I, Y) \times \{\id_Y\}} \arrow[d, hook]           \\
                {\Mul_{\O}(X_I, Y)}                                     & {\Mul_{\O}(X_I, Y) \times \Mul_{\O}((Y), Y)} \arrow[l, hook]
            \end{tikzcd}
        \end{center}
       
        \begin{center}
            \begin{tikzcd}
                {\Mul_{\O}(X_I, Y)} \arrow[r, "\cong"] \arrow[d, "\id"] & {(\prod_{i \in I} \{\id_{X_i}\}) \times \Mul_{\O}(X_I, Y)} \arrow[d, hook]           \\
                {\Mul_{\O}(X_I, Y)}                                     & {(\prod_{i \in I} \Mul_{\O}((X_i), X_i)) \times \Mul_{\O}(X_I, Y)} \arrow[l, hook]
            \end{tikzcd}                                             
        \end{center}
        ...
        \begin{adjustbox}{max width=\textwidth}
            \begin{tikzcd}
                {\prod_{j \in J} \Mul_{\O}(X_{I_j}, Y_j) \times \prod_{k \in K} \Mul_{\O}(Y_J, W_k) \times \Mul_{\O}(W_K, Z)} \arrow[rd, "\id \times \gamma"] \arrow[dd, "(\gamma \circ s) \times \id"] & \\
...
\begin{rem}
    When we drop the symmetric structure we obtain a definition of a colored operad which we call colored 
    planar operad. In other words we consider the indexing sets $I$ in definition \ref{coloroperad} to be finite 
...
\begin{egs}
    \begin{enumerate}
        \item A colored operad with one color is a symmetric operad in $\Set$ as defined in definition \ref{operad}. 
        ...
        in definition \ref{planaroperad}. Let $\P$ be a colored planar operad $\P$. We set for $n \in \N$:
        \[
         \P(n) \coloneqq \Mul_{\P}((X)_{1\leq i \leq n}, X)
        \]
        We obtain the composition map and operadic unit similarly as in the example for a one colored 
...
\begin{egs}
    \begin{enumerate}
        ...
        As it will be a generalization of proposition \ref{m1m2} and proposition \ref{SymMonBijection} illustrates how to reconstruct 
...
Let $\O$ be a colored operad. Then similarly as for the definition \ref{Cxdef} we have a forgetful functor 
\[
\pi \colon \Ox \to \Fin
\]
from which we can reconstruct the underlying colored operad $\O$ (up to canonical equivalence):
First, note that in general $\pi$ does not have to be a op-fibration. For an arbitrary morphism 
...
$\pi(f)=\alpha$ as in the proof of proposition \ref{m1m2}. 
...
$\phi \in \Mul_{\O}(\{X\}, Y)$.
Furthermore, the equivalence $\O_{\langle n \rangle} \cong \O^n$ holds as the weaker op-fibration condition (M1) property 
still suffices to admit the proof given in proposition \ref{m1m2}. 

...
We elaborate in the following why this did not work out as we hoped originally.
...
\begin{rem}
    ...
    First, we consider $p_1, \dots, p_n$ as $n$ strands such that we can braid by $b_g$. Then, 
    ...
    block braiding on $m$ strands. Each pencil corresponds to a block having the length of the 
...
The construction of the braided monoidal structure should be analogous to the symmetric case.
...
Thus, similarily to the symmetric case we would want to lift $\sigma_0$ using the op-fibration condition.
...
final candidate for an op-fibration.
...
each $$X \in \Ox_{[n]} \cong \O^n,$$ so the braid group $B_n$ acts on $\O^n$. However, we would expect to only

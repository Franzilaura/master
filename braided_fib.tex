...
will see how to reconstruct the structure of a colored braided operad from such a
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        Note, that for the same construction for $\widetilde{\eta}^{-1}$, we obtain $\bar{\eta}^{-1}$.
...
\begin{proof}
    ...
    \rangle$, we get by (iii) that there is an object $X \in \D_{\langle n
    ...
    For the following triangle in $\Br$:
            ...
            \end{tikzpicture}
        \end{center}
    along with collection of colors $\{X_i\}_{i \in I}, \{Y_j\}_{j \in J}$ and
    color $Z$, we define a composition map via the composition in $\D$ as
    follows: For chosen bijections of $I \cong \langle n \rangle^{\circ}$, $J
    ...
    bijections. Furthermore, note that by conditions (iv) and (v), for  colores
    $\{X_i\}_{i \in \alpha_j}$ there is an object $X^{(\alpha_j)}$ in
    $\D_{\langle |\alpha_j| \rangle}$ and morphisms $X^{(\alpha_j)} \to X_i$
    covering $rho^i$. For $\{X^{(\alpha_j)}\}_{1 \leq j \leq m}$ there is also
    an object X with morphisms $X \to X^{(\alpha_j)}$ covering $\rho^j$, $1 \leq
    j \leq m$. We observe that $X$ with the composite morphism $X \to X_i$ cover
    $\rho^i$. The following composition in $\D$:
    \[
    ...
    % $\D$.
    \todo{What about different choices of X?}

...
functor fulfilling these properties.
...
\begin{prop}
    ...
    \begin{enumerate}
        ...
        \rangle}$$.
        \item For object $X,Y$ in $\Bx$ the functor
        \[
...
\begin{proof}
    ...
    \begin{enumerate}
        \item \todo{nur cocartesian lift für inerts bisher gezeigt}Let
        $\widetilde{\alpha} \colon \langle n \rangle \to  \langle m \rangle$ be
        an inert morphism in $\Brx$, i.e. for a restriction to the active subset
        of $\langle n \rangle$, there is a bijection of finite sets:
        \[
        ...
        \begin{center}
            ...
            \end{tikzcd}
        \end{center}
        \item \todo{korrigieren, ich hab op-fib gezeigt}Let
        $\phi \colon [X_1, \dots, X_n] \to [Y_1, \dots, Y_m]$ be a morphism in
        $\Bx$ with $\pi(\phi)=\alpha_{\phi}$ and $\eta \colon \alpha_{\phi}
        \Rightarrow \alpha$ a $2$-morphism in $\Brx$. Then, we define a
        cartesian lift
        \[
            \psi \colon [X_1, \dots, X_n] \to [Y_1, \dots, Y_m]
        \]
        as follows: The underlying morphism we chose to be
        $\alpha_{\psi}=\alpha$. Furthermore, the $2$-morphism $\eta$ gives rise
        ...
        \begin{center}
            \begin{tikzpicture}
                ...
                % Natural transformation arrow
                \node at (1.5, 1.2) {$\overset{\eta_j}{\Leftarrow}$};
                % \node[rotate=-40] at (1.5, 1.2) {$\Downarrow \eta$};
            \end{tikzpicture}
        \end{center}
        ...
        \]
        We need to see that $\eta \phi \to \psi$ is indeed cocartesian. This
        however is true due to the fact that all $2$-morphisms have inverses in
        $\Bx$.
        % Let $\epsilon \colon \chi \Rightarrow \psi$ be another
        % $2$-morphism in $\Bx$ and $\widetilde{\delta}$ $2$-morphism in $\Brx$
        % such that $\widetilde{\eta} \circ \delta = \widetilde{\epsilon}$. Note,
        % that the following hold for all $1 \leq j \leq m$:
        % \[
        ...
        % \end{align*}
        % So, $\epsilon = \eta^{-1} \circ \chi$ can be lifted to a $2$-morphism in
        % $\Bx$.
        % \item The third property follows immediately from the construction on
        % $\Bx$. Since for morphisms $\phi, \psi \colon [X_1, \dots, X_n] \to
        % [Y_1, \dots, Y_m]$ a $2$-morphism $\eta \colon \phi \to \psi$ with
        % underlying $2$-morphism $\widetilde{\eta}$ is defined by
        % $\widetilde{\eta}_j$ for all $1 \leq j \leq m$.
...
We introduce from \cite[Corollary 2.4.6.5]{hht} the following notion of
categorical fibrations:
\begin{defin}\label{def:cat-fib-infty} Let $p \colon \C \to \D$ be a map of
    simplicial sets, where $\D$ is an $\infty$-category. We call $p$ a
    categorical fibration if the following conditions hold:
    \begin{enumerate}[(i)]
        \item The map $p$ is an inner fibration as defined in
        \cite[\href{https://kerodon.net/tag/01BA}{Definition 01BA}]{kerodon},
        i.e. every lifting problem for $0 < i < n$:
        \begin{center}
            \begin{tikzcd}
...
For a functor of $\Grp$-enriched categories this breaks down to the following
definition:
\begin{defin}\label{def:cat-fib-grpd} Let $p \colon \C \to \D$ be a functor of
    ...
    \begin{enumerate}[(i)]
        ...
        % admits a solution.
        \item For all object $X,Y$ in $\C$ the functor \todo{dont understand why
        (i) above translates into this}
        \[
...
\begin{rem}
    ...
    \langle m \rangle$ in $\Brx$ is an inert morphism as the restriction to
    $\alpha^{-1}(\langle m \rangle^{\circ})$ has to be a bijection.
    \todo{is this true? isn't bijection on image enough?x}
    Thus, condition (ii) from Definition \ref{def:cat-fib-grpd} for a functor
    $p$ translates to:
    \begin{itemize}
    \item For every inert morphism $\alpha \colon \langle n \rangle \to \langle
    m \rangle$ in $\Brx$ and object $X \in \D$ there exists a $p$-cocartesian
    morphism $f \colon X \to X'$ lifting $\alpha$.
    \end{itemize}
\end{rem}
...
\begin{defin}
    ...
    \begin{enumerate}[(i)]
    % \item For every inert morphism $\alpha \colon \langle n \rangle \to
    % \langle m \rangle$ in $\Brx$ and object $X \in \D$ there exists a
    % $\pi$-cocartesian morphism $f \colon X \to X'$ lifting $\alpha$. In
    % particular, $\alpha$ induces a functor: $$\pi_{\alpha} \colon \Bx_{\langle
    % n \rangle} \to \Bx_{\langle m \rangle}.$$
    \item The functor $\pi$ is a categorical fibration by Definition
    \ref{def:cat-fib-grpd}. In particular, $\alpha$ induces a functor:
    $$\pi_{\alpha} \colon \D_{\langle n \rangle} \to \D_{\langle m \rangle}.$$
    \item Let $X \in \D_{\langle n \rangle}$ and $X' \in \D_{\langle m \rangle}$
    be objects, $\alpha : \langle n \rangle \to \langle m \rangle$ be a morphism
    in $\Brx$ and let $\Hom_{\D}^{\alpha}(X, X')$ be the 
    % isomorphism class of 
    morphisms which lie over $\alpha$. 
    
    Choose $\pi$-cocartesian morphisms $X' \to X'_i$ lying over the inert
    morphism $\rho^i \colon \langle n \rangle \to \langle 1 \rangle$ for $1 \leq
    i \leq n$. Then the induced map
    \[
        \Hom_{\D}^{\alpha}(X, X') \to \prod_{1 \leq i \leq n} \Hom_{\D}^{\rho^i \circ \alpha}(X, X_i')
    \]
    is an equivalence of groupoids. 
...
\end{prop}
For this statement, we will only sketch a reconstruction of a colored braided
...
\begin{proof}
    ...
    \[
        \pi \colon \D \to \Brx
    \]
    be an operadic fibration. We will define a colored braided operad $\B$ from
    ...
    $Y$ a color. For a choice of bijection $\iota \colon I \cong \langle n
    \rangle$, we get by (iii) that there is an object $X \in \D_{\langle n \rangle}$ 
    and a collection of morphisms $X \to X_i$ covering $\rho^i$ for $i \in I$. Then, 
    \[
    ...
    \todo{now just take union of these sets?}

    The composition map is induced by the composition map in $\D$. 
    \todo{define composition}
    ...
    unitality in $\D$. This construction gives raise to an equivalence of

...
\begin{proof}
    ...
    The identity morphisms in $\B$ are given by
    the identity morphisms in the fiber category $\D_{\langle 1 \rangle}$, i.e.,
    for each color $X \in \D_{\langle 1 \rangle}$, the identity morphism $\id_X$
    in $\D$ defines the identity morphism in $\Mul_{\B}^{\id}(\{X\}, X)$.
    The associativity requirement is met as the composition if defined via the
    composition map in $\D$ which is associative.
    The unitality condition is also induced by the unitality in $\D$.
    \todo{equivalence}
\end{proof}